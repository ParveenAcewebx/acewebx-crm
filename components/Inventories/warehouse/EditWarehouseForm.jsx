...
const EditWarehouseForm = ({ editLeadId }) => {
  ...
  useDocumentTitle('Edit warehouse')
  const router = useRouter()

  // Fetch the lead by Id To update the Id
  const fetchWarehouseById = async () => {
    try {
      const response = await InventoryServices.getWarehouseById(id)
  ...
  // handle to edit
  const handleWarehouseUpdate = async data => {
    try {
      ...
      const responseEdit = await InventoryServices.updateWarehouseById(
        id,
        formData
      )
  ...
  return (
    <>
      <div className='flex justify-between'>
        <LayoutHeader pageTitle={'Edit Warehouse'} />
      </div>
      ...
      <div className=''>
        <FormProvider {...form}>
          <form
            className=''
            onSubmit={form.handleSubmit(handleWarehouseUpdate)}
          >
            <AddWarehouseForm form={form} id={id} />
