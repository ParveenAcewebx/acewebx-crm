...
import BirthdaysApi from '@/services/cadidateApis/employees/BirthdaysApi'
...
function EditBirthday() {
  ...
  const onSubmit = async (data) => {
    setLoader(true)
    try {
      const response = await BirthdaysApi.editBirthdays(id, eventId, data)
  ...
  const candidateDataGetById = async () => {
    try {
      const response = await BirthdaysApi.getByIdBirthdays(id, eventId)
      if (response?.data?.status === true) {
