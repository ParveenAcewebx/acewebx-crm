...
function IncrementChatCompo({ id }) {
  ...
  const messageSendHandler = async () => {
    ...
    try {
      await IncrementAPi.addMessageIncrement(data)
      form.reset()
      await getAllChats()
    } catch (error) {
  ...
  const editMsgHandler = async () => {
    ...
    try {
      await IncrementAPi.editMessageIncrement(editMsgId, message)
      form.reset()
      setIsEditMsg(false)
      setEditMsgId(null)
      await getAllChats()
  ...
  const getByIdMsgHandler = async id => {
    try {
      setIsEditMsg(true)
      setEditMsgId(id)
      const oldMsg = await IncrementAPi.getByIdMessageIncrement(id)
  ...
  const deleteMsgHandler = async id => {
    try {
      await IncrementAPi.deleteMessageIncrement(id)
      await getAllChats()
    } catch (error) {
  ...
  const getAllChats = async () => {
    try {
      const allMsg = await IncrementAPi.getAllMessagesIncrement(id)
...
export default IncrementChatCompo
