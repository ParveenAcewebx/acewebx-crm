...
const VendorList = () => {
  ...
  const getListBudgetBook = async () => {
    try {
      setLoading(true)
      const res = await InventoryServices.AllVendors(page, length)
      if (res?.status === 200) {
  ...
  const onDelete = async () => {
    if (deleteIndex !== null) {
      try {
        const res = await InventoryServices.deleteVendors(deleteIndex)
        console.log('res', res)
  ...
  const handleEditContact = row => {
    router.push(`/dashboard/inventory/vendor/edit?id=${row.original.id}`)
  }
  ...
  return (
    <>
      <div className='mb-3 flex items-center justify-between'>
        <LayoutHeader pageTitle='Vendor List' />
        <Button
          className='site-button'
          onClick={() => router.push(`/dashboard/inventory/vendor/add`)}
        >
          <Plus />
          Add Vendor
        </Button>
      ...
      <DataTable
        data={getList}
        loading={loading}
        columns={VendorColumns(handleDeleteContact, handleEditContact)}
