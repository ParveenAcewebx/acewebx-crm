  ...
  const candidateDataGetById = async (editId) => {
    try {
      ...
      if (response?.data?.status === true) {
        ...
        // Extracting fields and preparing object for form.reset
        const dataForSet = {
          // Personal Info
          name: data?.name || '',
          ...
          // Documents
          aadharCard: meta?._aadharCard || '', // Adjust if present
          panCard: meta?._panCard?.filePath || '',
  
          // Banking Details
          bankName: meta?._bankName || '',
          bankAccountNumber: meta?._bankAccountNumber || '',
          bankIFSC: meta?._bankIFSC || '',
          ...
          emergencyContactRelation: meta?._emergencyContactRelation || ''
        };
