...
import { ProductTagsColumns } from './productTags-columns'
import AddProductTags from '@/components/ItemSettings/ProductTags/AddProductTags'
import EditProductTags from '@/components/ItemSettings/ProductTags/EditProductTags'
...
const ProductTagsList = () => {
  ...
  const getAllList = async () => {
    try {
      setLoading(true)
      const res = await ItemSettingsServices.AllProductTags(page, length)
      if (res?.status === 200) {
  ...
  const onDelete = async () => {
    if (deleteIndex !== null) {
      try {
        const res = await ItemSettingsServices.deleteProductTags(deleteIndex)
        console.log('res', res)
  ...
  return (
    <>
      <div className='mb-3 flex items-center justify-between'>
        <LayoutHeader pageTitle='Product Tags List' />
      ...
      <DataTable
        data={getList}
        loading={loading}
        columns={ProductTagsColumns(handleDeleteContact, handleEditContact)}
      ...
      <LeadSettingModal
        ...
        message={editData ? 'Edit Product Tags' : 'Add Product Tags'}
      />
    </>
  )
}
