...
import AddProductCategory from '@/components/ItemSettings/ProductCategory/AddProductCategory'
import EditProductCategory from '@/components/ItemSettings/ProductCategory/EditProductCategory'
...
const ProductCategoryList = () => {
  ...
  const getAllList = async () => {
    try {
      setLoading(true)
      const res = await ItemSettingsServices.AllProductCategories(page, length)
      if (res?.status === 200) {
  ...
  const onDelete = async () => {
    if (deleteIndex !== null) {
      try {
        const res =
          await ItemSettingsServices.deleteProductCategories(deleteIndex)
        console.log('res', res)
  ...
  return (
    <>
      <div className='mb-3 flex items-center justify-between'>
        <LayoutHeader pageTitle='Product Category List' />
      ...
      <LeadSettingModal
        ...
        message={editData ? 'Edit Product Category' : 'Add Product Category'}
      />
    </>
  )
}
