'use client'
import AddCategory from '@/components/BudgetBookScope/budget-category/AddCategory'
import EditCategory from '@/components/BudgetBookScope/budget-category/EditCategory'
...
const BudgetCategoryList = () => {
  ...
  const getAllList = async () => {
    try {
      setLoading(true)
      const res = await BudgetScopeService.GetAllCategory(page, length)
      if (res?.status === 200) {
  ...
  const onDelete = async () => {
    if (deleteIndex !== null) {
      try {
        const res = await BudgetScopeService.DeleteCategoryById(deleteIndex)
        console.log('res', res)
  ...
  return (
    <>
      ...
      <LeadSettingModal
        ...
        message={editData ? 'Edit Category' : 'Add Category '}
      />
    </>
  )
}
