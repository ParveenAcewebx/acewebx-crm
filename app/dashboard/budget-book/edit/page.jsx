...
const EditBudgetBook = () => {
  ...
  const methods = useForm({ defaultValues: BudgetBookDefaultValues })
  const router = useRouter()
  const lead = leadName?.project?.name

  const { control, setValue, watch, getValues } = methods

  useEffect(() => {
    if (editData?.uploadDocument) {
      setTableData(editData.uploadDocument)

      editData.uploadDocument.forEach(item => {
        methods.setValue(`note-${item.id}`, item.notes || '')
      })
    }
  }, [editData])
  // fetch budget category all  value for budget total
      ...
      if (response?.status === 200) {
        ...
        const formattedData = {
          ...
          projectScopeIncludes: budgetData?.projectScopeIncludes?.map(item => ({
            ...
            is_exclude: item?.is_exclude === 0 ? false : true
          })),
          ...
          // contact_id: budgetData.contact?.id || '',
  ...
  console.log('index', watchedSitesPlan)
  ...
  useEffect(() => {
    if (
      ...
      return
    console.log('watchedSites', watchedSites)
  ...
  // Helper to convert booleans to 1/0 in object values
  ...
  // Merge imageUpload and tableData
  const combinedUploadData = [
    ...
    ...(tableData || []).map(item => ({
      ...
      note: item.notes || '',
      type: item.type || '',
      displayToCustomer: item.displayToCustomer ?? item.is_display ?? 0
    }))
  ]

  const onUpdate = async () => {
    const updateType = methods.getValues('update')
    const updateProjectScopes = budgetData.projectScopes?.map(item => ({
      ...item,
      is_include: 1
    }))
    ...
    combinedUploadData.forEach((doc, index) => {
      const file = doc.file

      if (file instanceof File) {
        formData.append(`uploadDocument[${index}][file]`, file)
      } else {
        // It's an existing file, just send metadata
        formData.append(
          `uploadDocument[${index}][file][path]`,
          file?.path || ''
        )
        formData.append(`uploadDocument[${index}][file][id]`, file?.id || '')
      }
    ...
    // 3. Append the rest of the form config
    const configData = {
      engineer_id: 3,
      ...budgetData,
      ...
      projectContracts: convertBooleansToNumbers(
        budgetData.projectContracts || []
      ),
      projectKeyAreas: convertBooleansToNumbers(
        budgetData.projectKeyAreas || []
      ),
      projectScopeIncludes: convertBooleansToNumbers(
        budgetData.projectScopeIncludes || []
      ),
      projectSubmittals: convertBooleansToNumbers(
        budgetData.projectSubmittals || []
      )
    }

    formData.append('form', JSON.stringify(configData))
