...
'use client'
import UploadedDocument from '@/app/dashboard/budget-book/preview/UploadDocument'
...
const QuotePreview = () => {
  ...
  const secretId = searchParams.get('custm')
  ...
  const emailTrue = verifyEmail == 'true'
  const form = useForm()
  ...
  const getInvoice = async () => {
    try {
      const response = selectdRevision
        ...
        : await api.get(`/get-projects/${encryptedId}`)
  ...
  const onSubmitEmail = async data => {
    ...
    try {
      const response = await api.get(
        `/verify-lead-mail/${secretId}/${data?.email}`
      )
      if (response?.status == 200) {
        successMessage({ description: response?.data?.message })
        setVerifyEmail(String(response?.data?.status))
      }
    } catch (error) {
  ...
  useEffect(() => {
    if (emailTrue||selectdRevision) {
      getInvoice()
    }
  }, [selectdRevision,emailTrue])
  ...
  const getNotesApi = async () => {
    try {
      const res = await api.get(`get-lead-notes`)
      if (res?.status === 200) {
                        ...
                        <td colSpan={2}>
                          ...
                          >
                            <tr style={{ textAlign: 'center' }}>
                              ...
                              >
                                <em>
                                  *ANY SHIPMENTS ABOVE OR SEALS ABOVE ARE
                      ...
                      <tr>
                        ...
                        >
                          ...
                          <p>
                            ...
                            charges may apply for non-typical shipments or
                            changes to scope. Approval must be received before
                            order release. Orders will not be accepted unless
                            signed by builder/GC. Builder must notify Blue Sky
                            at time of approval if the quote is being used as an
                            alternate. Orders must include the latest approved
                            drawing(s). Blue Sky is not liable for errors caused
                            by builder changes not reflected in approved
                            submittals.
                          </p>
                        </td>
                      </tr>
